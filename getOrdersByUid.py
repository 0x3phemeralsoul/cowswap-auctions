...
def main():
    ...
    orders = session.query(Order.orderHash).distinct().all()
    
    # Fetch API data for each transaction
    for order in orders:
        order_hash = order.orderHash
        exists = session.query(OrderByUid.uid).filter_by(uid=order_hash).first() is not None
        if(not exists):
            ...
            print(f"Order_hash {order_hash} already in DB, skipping.")
        # Close the session
        session.close()
