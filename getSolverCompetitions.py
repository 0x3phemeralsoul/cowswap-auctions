...
def fetch_api_data(tx_hash):
    ...
    if response.status_code == 200:
        ...
        print(f"Error fetching API data for tx_hash {tx_hash}. Status Code: {response.status_code}")
        return None
...
def main():
    ...
    logger.add("logs/getSolverCompetitions_{time}.log", level=os.getenv("LOGGER_LEVEL"), rotation="100 MB")
