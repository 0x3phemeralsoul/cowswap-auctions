...
class CallDataToken(Base):
    __tablename__ = 'call_data_tokens'

    id = Column(Integer, primary_key=True)
    address = Column(String, unique=True, nullable=False)

class CallDataClearingPrice(Base):
    __tablename__ = 'call_data_clearing_prices'

    id = Column(Integer, primary_key=True)
    price = Column(String, nullable=False)

    solution_id = Column(Integer, ForeignKey('solutions.id'))
    solution = relationship('Solution', back_populates='CallDataClearingPrice')

class CallDataTrade(Base):
    __tablename__ = 'call_data_trades'
    ...
    """ sell_token_id = Column(Integer, ForeignKey('call_data_tokens.id'))
    sell_token = relationship('CallDataToken', back_populates='CallDataTrade')

    buy_token_id = Column(Integer, ForeignKey('call_data_tokens.id'))
    buy_token = relationship('CallDataToken', back_populates='CallDataTrade') """

    solution_id = Column(Integer, ForeignKey('solutions.id'))
    solution = relationship('Solution', back_populates='CallDataTrade')

class CallDataInteraction(Base):
    __tablename__ = 'call_data_interactions'

    id = Column(Integer, primary_key=True)
    target = Column(String, nullable=False)
    value = Column(String, nullable=True)
    call_data = Column(String, nullable=True)
    interactionPos = Column(Integer)

    """ target_token_id = Column(Integer, ForeignKey('call_data_tokens.id'))
    target_token = relationship('CallDataToken', back_populates='CallDataInteraction') """

    solution_id = Column(Integer, ForeignKey('solutions.id'))
    solution = relationship('Solution', back_populates='CallDataInteraction')
...
for price in clearing_prices_data:
    clearing_price = CallDataClearingPrice(price=price)
...
for trade_data in example_trades_data:
    trade = CallDataTrade(**trade_data)
    session.add(trade)
...
example_interactions_data = [
    {
        ...
        'value': 0,
        'call_data': b'\xe4I\x02.\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01Mk\xaa%K\xadU\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x07\xb7v<\x06w\xd2'
    },
    {
        'target': '0x162bb2Bb5FB03976a69dD25bB9afcE6140dB1433',
        'value': 0,
        'call_data': b'\xa9\x05\x9c\xbb\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x86\xf5\xd5\x9ahqp\xcfC\x15\x00\xd7\x00q\x06\x00\\\\\xec.\xe9\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x07\xb7v<\x06w\xd2'
    },
    # Add more interactions if needed
]

for interaction_data in example_interactions_data:
    interaction = CallDataInteraction(**interaction_data)
...
session.query(CallDataToken).delete()
session.query(CallDataClearingPrice).delete()
session.query(CallDataTrade).delete()
session.query(CallDataInteraction).delete() """
